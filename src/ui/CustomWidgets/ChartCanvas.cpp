    ...
    try
    {
        for (int i = 0; i < notes.size(); ++i)
        {
            ...
            if (note.type == NoteType::RAIN)
            {
                ...
                if (endNoteTime < startTime || noteTime > endTime)
                    continue;
            ...
            {
                if (noteTime < startTime - 100 || noteTime > endTime + 100)
                    continue;
                bool selected = m_selectionController && m_selectionController->selectedIndices().contains(i);
...
void ChartCanvas::wheelEvent(QWheelEvent *event)
{
    ...
    if (delta != 0)
    {
        ...
        if (m_chartController && m_chartController->chart())
        {
            ...
            if (m_verticalFlip)
            {
                baselineBeat = m_scrollBeat + (1.0 - baselineRatio) * m_visibleBeatRange;
            }
            else
            {
                baselineBeat = m_scrollBeat + baselineRatio * m_visibleBeatRange;
            }
    ...
    if (event->angleDelta().x() != 0)
    {
        ...
        if (m_visibleBeatRange < 0.1)
            m_visibleBeatRange = 0.1;
        if (m_visibleBeatRange > 1000)
            m_visibleBeatRange = 1000;
        update();
    }
